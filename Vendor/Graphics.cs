...
//using System.Windows.Threading; F
...
namespace LayerPaint
{
    ...
    public class Graphics : IGrap
    {
        Task task;

        int ww, hh;
        public Graphics(Color c, WriteableBitmap w)
        {
        ...
        public Graphics() : this(Color.Black, new WriteableBitmap(1, 1)) { }
        /*
        public WriteableBitmap Bitmap
        {
            ...
            set
            {
                bitmap = value;
                ww = value.PixelWidth;
                hh = value.PixelHeight;
                sm = bitmap.PixelBuffer.AsStream(); 
                SStream = Stream;
            }
        }
        */
        ...
        byte[] din;
        byte[] dtmp;
        public void fillCircle(Color cc, double x, double y, double r, double b, double a, double d)
        {
            double r2 = r * r;
            double _r = 1 / (r);         //极限优化
            ...
            for (int l = _l; l < len; l++)
            {
                ...
                for (int s = _s, pos = lp; s < slen; s++)//s<slen 防止划出界
                {
                    ...
                    if (p2 < r2)
                    {
                        ...
                        OutByte[pos] = (byte)(dtmp[pos] * a + din[pos] * _a);
                        if (OutByte[pos] != 255)
                        {
                        ...
                        OutByte[pos] = OA;// (byte)(dtmp[pos] * a + din[pos] * _a);
                        pos++;
                    }
                    else pos += 4;
                }
        ...
        public void DrawCircle(Vec2 p0, float size, float hard, float alpha, float dep = 1f, float blend = 0f, float di = 0f)
        {
        ...
        public Task DrawBucker(int x, int y,byte[] MaskByte)
        {
            return task = task.ContinueWith(_ => {
                floodFill(x, y , MaskByte);
                Invalidate();
            });
        }
